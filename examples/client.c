...
int client_main(int argc, char* argv[])
{
	...
	result = mbedtls_x509_crt_parse(&cacert, (const unsigned char*)mbedtls_test_cas_pem,
	...
	if ((result = mbedtls_net_connect(&server_fd, SERVER_ADDR,
	...
	if ((result = mbedtls_ssl_set_hostname(&ssl, SERVER_NAME)) != 0)
	{
	 ...
	 * MBEDTLS_SSL_VERIFY_OPTIONAL, we would bail out here if result != 0 */
...
send_request:
	...
	do result = mbedtls_ssl_write(&ssl, (unsigned char*)MESSAGE, len);
	while (result == MBEDTLS_ERR_SSL_WANT_READ ||
		result == MBEDTLS_ERR_SSL_WANT_WRITE);
	...
	do result = mbedtls_ssl_read(&ssl, buf, len);
	while (result == MBEDTLS_ERR_SSL_WANT_READ ||
		result == MBEDTLS_ERR_SSL_WANT_WRITE);
...
close_notify:
	...
	do result = mbedtls_ssl_close_notify(&ssl);
